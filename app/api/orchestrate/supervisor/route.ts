...
export async function POST(
  ...
  const rl = checkRateLimit(`orchestrator:post:${request.headers.get('x-forwarded-for') ?? 'unknown'}`, 'default');
  if (!rl.allowed) {
    return NextResponse.json(
      { success: false, error: { code: 'RATE_LIMIT', message: 'Too many requests' }, meta: { timestamp: new Date().toISOString(), executionTime: 0 } },
...
export async function GET(request: NextRequest): Promise<NextResponse<OrchestratorResponse>> {
  const rl = checkRateLimit(`orchestrator:get:${request.headers.get('x-forwarded-for') ?? 'unknown'}`, 'default');
  if (!rl.allowed) {
    return NextResponse.json(
      { success: false, error: { code: 'RATE_LIMIT', message: 'Too many requests' }, meta: { timestamp: new Date().toISOString(), executionTime: 0 } },
      { status: 429, headers: corsHeaders },
  ...
  return NextResponse.json(
    {
      ...
      data: {
        ...
        endpoints: [
          {
            method: 'POST',
            path: '/api/orchestrate/supervisor',
