...
export function PortfolioCard({ item, onClick, index }: Props) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl overflow-hidden cursor-pointer transition-all group"
    >
      {item.coverUrl ? (
        ...
        <div className="h-40 bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center">
          <span className="text-zinc-600 text-4xl font-bold">{item.clientName.charAt(0)}</span>
        </div>
      )}
      <div className="p-4">
        ...
        <p className="text-xs text-zinc-500 mb-2">{item.clientName}</p>
        <div className="flex items-center gap-2 mb-3">
          <PortfolioTypeBadge type={item.projectType as any} />
          {!Boolean(item.published) && <span className="text-[10px] text-zinc-600 bg-zinc-800 px-1.5 py-0.5 rounded">Brouillon</span>}
        </div>
