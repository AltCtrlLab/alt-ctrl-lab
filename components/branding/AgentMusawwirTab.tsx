...
export function AgentMusawwirTab() {
    ...
    <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {AGENT_BRIEF_EXAMPLES.musawwir.map((example) => (
            <button key={example} type="button" onClick={() => setBrief(example)}
              className="text-xs px-3 py-1.5 rounded-lg bg-fuchsia-500/10 border border-fuchsia-500/20 text-fuchsia-300 hover:bg-fuchsia-500/20 transition-all text-left">
      ...
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-5 space-y-3">
        ...
        <textarea value={brief} onChange={e => setBrief(e.target.value)} disabled={submitting}
          placeholder="Décris ton brief à Abdul Musawwir…" rows={4}
          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 placeholder-zinc-600 resize-none focus:outline-none focus:border-fuchsia-500/40 disabled:opacity-50" />
        <button onClick={submitBrief} disabled={submitting || !brief.trim()}
          className="flex items-center gap-2 px-4 py-2 bg-fuchsia-500/10 hover:bg-fuchsia-500/20 border border-fuchsia-500/25 text-fuchsia-300 text-sm font-medium rounded-xl transition-all disabled:opacity-40">
      ...
      <section>
        ...
        {loading ? (
          ...
          <EmptyState icon={Palette} color="fuchsia" message="Aucun brief en cours" submessage="Envoie un brief ci-dessus." />
        ) : (
          <div className="space-y-3">
            {activeTasks.map(task => (
              <div key={task.id} className="bg-fuchsia-500/[0.08] border border-fuchsia-500/25 rounded-2xl p-4 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin text-fuchsia-400" /><span className="text-sm font-medium text-fuchsia-300">{STAGE_LABELS[task.status] ?? task.status}</span></div>
              ...
              return (
                <div key={task.id} className={`rounded-2xl p-4 space-y-3 border ${ok ? 'bg-emerald-500/[0.08] border-emerald-500/25' : 'bg-red-500/[0.08] border-red-500/25'}`}>
                  ...
                  {(task.result || task.error) && (
                    <button onClick={() => toggleResult(task.id)} className="text-xs text-zinc-400 hover:text-zinc-300 underline underline-offset-2">{isExpanded ? 'Masquer' : 'Voir le résultat'}</button>
                  )}
